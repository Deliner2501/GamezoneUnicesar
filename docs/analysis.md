...
**Answer:** Sale relates to Customer, Seller, and Product through associations, because a sale uses objects from those classes but does not inherit from them. A sale belongs to a customer, is handled by a salesperson, and contains one or more products. The relationship between Sale and its products can be represented as composition because the included products are considered integral elements of the transaction.
...
**Answer:** The model layer contains business entities; the persistence layer contains classes that save and retrieve information from files; the service layer contains business rules; and the ui layer contains the console menu and user interaction. A class's placement depends on the responsibility it performs, avoiding the mixing of responsibilities across layers.
...
**Answer:** The permitted dependencies between layers are Ui -> Service, where the user interface uses services to execute operations; Service -> Model and Persistence, where services use model entities and access the persistence layer to save or retrieve information; and Persistence -> Model, since the persistence layer works with domain entities. The Model layer does not depend on any other layer, as it must remain independent. Dependencies that violate this structure are prohibited. For example, the Ui accessing the Persistence directly, or the Model depending on the Ui, Service, or Persistence. This arrangement allows for the separation of responsibilities, reduces coupling, and facilitates system maintenance.